...
struct Battlefield {
						...
						for (int m = 0; m < 9; m++)
						{
							for (int n = 0; n < 9; n++)
							{
								if (current->possible_field[j][m][n] == true && current->gamefield[m][n] == forest) {
									...
									current->possible_field[j][y[0]][n] |= (current->gamefield[y[0]][n] == forest);
									current->possible_field[j][y[1]][n] |= (current->gamefield[y[1]][n] == forest);
									current->possible_field[j][m][x[0]] |= (current->gamefield[m][x[0]] == forest);
									current->possible_field[j][m][x[1]] |= (current->gamefield[m][x[1]] == forest);
								}
							}
						}
					}
	...
	bool genBranch(int move_limit = 2, int enemy_possible_pos = 2) {
		//stay or move
		...
		if (steps_in_forest[0] || steps_in_forest[1]) {
				...
				for (auto & j : act[1])
				{
					...
					for (auto & k : act[2])
					{
						for (auto & l : act[3])
						{
							...
							if (current.lose() || tank_in_danger[0] || tank_in_danger[1] || killed[0] || killed[1]) {
								cut = true;
								break;
							}
							current.eval_field();
