...
namespace RedBot {
    ...
    double _opposite_can_fire_advantage = 2; //0.5-5
    double _dead_end_punishment = 35;
    //[evaluate]
    double _self_distance_weight = 1.2; //1-2
    double _sgn_exp_weight = 1.2;
    double _depth_weight = 0.5; //纵深权重 0.1 - 0.6
    double _self_near_base_weight = 2.0; //用于扩大离敌方基地近的优势 1-5
    double _self_near_base_border = 7; // 算作"离敌方基地近”的标准 4-7 //!
    double _can_fire_advantage = 0.5;
    //[multipletank]
    double _multitank_advantage_border = 1; // 比敌方近*步，算作有明显优势
    double _multitank_far_punishment = 5; // 没明显优势，苟
    ...
    double _cost_attack_weight = 0.5; //0.2-1.2
    ...
    inline int Manhattan(int x1, int y1, int x2, int y2) {///cmath里面竟然没有符号函数
...
///卡大小开数组不是好文明
    ...
    struct Battlefield {
        ...
        bool dead_end(Tank &tmpTank) {
            ...
            if (isOpposite(tmpTank, tank[1 - tmpside][0]) or isOpposite(tmpTank, tank[1 - tmpside][1])) {
                ...
                if (tank[1 - tmpside][op].x == xx) {//敌方和自己在同一竖线上
                    if ((inMap(xx - 1, yy) and state[yy][xx - 1] == 0) or
                        (inMap(xx + 1, yy) and state[yy][xx + 1] == 0)) {//只要左右两边还有地方可以躲，开炮也成
                    ...
                    if ((inMap(xx, yy - 1) and state[yy - 1][xx] == 0) or
                        (inMap(xx, yy + 1) and state[yy + 1][xx] == 0)) {
                        //        memcpy(state, state_back, sizeof(state));//复原地图
                        return false;//不用剪枝
                    } else {
                        //         memcpy(state, state_back, sizeof(state));//复原地图
            ...
            for (unsigned i = 0; i < 2; i++) {
                ...
                if (act > 3) {
                    ...
                    while (inMap(xx, yy)) {
                        ...
                        if ((yy == tank[enemy][0].y and xx == tank[enemy][0].x) or
                            (yy == tank[enemy][1].y and xx == tank[enemy][1].x))
                            break;
        ...
        void quick_play(Action act[2]) {
            ...
            for (unsigned tmpside = 0; tmpside < 2; tmpside++) {
                for (unsigned tmp_id = 0; tmp_id < 0; tmp_id++) {
                ...
                if (act[enemy][id] >= 4) {
                    ...
                    while (inMap(xx, yy)) {
                        ...
                        if (tank[enemy][1 - id].at(xx, yy)) {///我 打 我 自 己
                            ...
                            } else if (!is_shoot(act[enemy][1 - id]) or (is_shoot(act[enemy][1 - id]) and
                                                                         act[enemy][id] !=
                                                                         opposite_shoot(act[enemy][1 - id]))) {
                        ...
                        for (unsigned i = 0; i < 2; i++) {
                            if (tank[self][i].at(xx, yy)) {
                                ...
                                if (tank[self][1 - i] == tank[self][i]) {
                                    ...
                                    ///到这一步可以确定不会有重叠的坦克
                                else if ((act[enemy][id] != opposite_shoot(act[self][i]) and is_shoot(act[self][i])) or
                                         !is_shoot(act[self][i])) {
                                    ///如果不是对射甚至己方没有射击
                ...
                if (act[self][id] >= 4) {
                    ...
                    while (inMap(xx, yy)) {
                        ...
                        if (tank[self][1 - id].at(xx, yy)) {///我 打 我 自 己
                            ...
                            } else if (!is_shoot(act[self][1 - id]) or (is_shoot(act[self][1 - id]) and act[self][id] !=
                                                                                                        opposite_shoot(
                                                                                                                act[self][
                                                                                                                        1 -
                                                                                                                        id]))) {
                        ...
                        for (unsigned i = 0; i < 2; i++) {
                            if (tank[enemy][i].at(xx, yy)) {
                                ...
                                if (tank[enemy][1 - i] == tank[enemy][i]) {
                                    ...
                                    tank_die[self][1 - id] = true;
                                } else if (!is_shoot(act[enemy][i]) or (is_shoot(act[enemy][i]) and act[self][id] !=
                                                                                                    opposite_shoot(
                                                                                                            act[enemy][id]))) {
        ...
        bool move_valid(int side, int id, int act) const {
            ...
            for (int i = 0; i < 2; i++) {
                if (tank[1 -
                         side][i].alive())//can not step into a enemy's tank's block (although tanks can overlap inadventently)
                {
        ...
        void locate_enemy(vector<Battlefield> &cases) const {
            ...
            for (unsigned i = 0; i < min(enemypos[0].size(), 4); i++) {
                for (unsigned j = 0; j < min(enemypos[1].size(), 4); j++) {
        ...
        int cost_attack(int side, int id) {
            ...
            queue<point> q;
            if (state[8 * dest_side][4] == base or state[8 * dest_side][4] == brick)
                distance[dest_side][8 * dest_side][4] = 1;
            ...
            while (!q.empty()) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if ((temp.y == tank[side][0].y and temp.x == tank[side][0].x) or
                        (temp.y == tank[side][1].y and temp.x == tank[side][1].x)) {
                        temp.cost += 1;
                    }
        ...
        inline double forest_eval(int tmpside = self) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (state[y][x] == forest) eval += _in_forest_reward;///para magic number
        ...
        inline double opposite_eval(int tmpside = self) {
            ...
            for (int i = 0; i < 2; i++) {
                if (opposite[i][0] & opposite[i][1]) {///自己被敌人夹击,几乎必死无疑
                    ...
                    eval -= _tank_oppposite_punishment;///para 又一个magic number 躲开总是好一点的
                    ...
                    eval += (double) (can_shoot[tmpside][i] - can_shoot[!tmpside][op]) * _opposite_can_fire_advantage;///鉴于tank2的特殊情况给了一个很小的值让它不至于太怂
                    if (can_shoot[!tmpside][op] and !can_shoot[tmpside][i]) {
                        if (dead_end(tank[tmpside][i])) {///如果坦克必死无疑
                            eval -= _dead_end_punishment;
                        }
                    }
                }
            }
        ...
        double evaluate(bool tmpside = self) {///针对某一方进行估值，默认对己
            ...
            int self_dis[2] = {35, 35}, enemy_dis[2] = {35,
                                                        35}, self_min = 10000, enemy_min = 10000;///己方 敌方两辆坦克距离 己方 敌方最短距离
            for (int i = 0; i < 2; i++) {
                if (!tank[tmpside][i].dead) {
                    if (!Tank::exposed[i]) val += 0.2;
                    self_dis[i] = cost_attack(tmpside, i);
                    val += 2.0 / (double) Manhattan(tank[tmpside][i].x, tank[tmpside][i].y, 4,
                                                    8 * (1 - tmpside));///这个是类似微小扰动的东西 不用调
                }
                if (!tank[!tmpside][i].dead)
            ...
            val += _sgn_exp_weight * sgn(enemy_min - self_min) * exp(enemy_min - self_min) /
                   min(min(self_min, enemy_min) + 1, 5);///不能让敌人太近
            val += _depth_weight * (double) (depth(tank[tmpside][1]) + depth(tank[tmpside][0]));///2s的纵深可以适当地价值更高
            ///这个价值可以适当地大一些 大概 1 —— 5 的范围 (geatpy)
            val +=  _self_near_base_weight * exp(_self_near_base_border - self_min);///离得越近向前走的价值就越大
            //cout << "p2:" << val << endl;
            ///关于坦克如何移动到此结束
            ///战斗部分
            ...
            for (int i = 0; i < 2; i++) {///己方i
                for (int j = 0; j < 2; j++) {///敌方j
                    if (overlap(tank[tmpside][i], tank[!tmpside][j])) {
                        if (self_dis[i] - enemy_dis[j] >= -_multitank_advantage_border ) val -= _multitank_far_punishment;///这里采取保守策略，尽可能不出现重叠
                            ///tank2s采取冲锋策略 tank2可以保证不输 但2s不行 这里赌一把
                        else val += _multitank_near_reward;///己方离得近的话大概可以冲了
                    }
                }
            }
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if(enemy_min > _intercept_border) continue;
                if (self_dis[i] > enemy_dis[1 - i]) {///只考虑和自己相对的坦克
                    val -= _intercept_punishment;///先扣分再说 加分不能超过总得分 除非算上cost attack
                    get_route(1 - i);
                    if (route[tank[self][i].y][tank[self][i].x]) {///自己在敌方坦克预计行动路线上
                        val += _on_route_reward;
                    } else if (intercept(i, 1 - i)) {///能够拦截敌人(至少震慑几回合
                        if (tank[self][i].n_fire_rounds > 0) val += _intercept_fire_reward;
                        else val += _intercept_stay_reward;
                    }
                    ///note：关于是否撤销离敌人距离的估值有待商榷 但是拦截状态确实无法对此作出有效贡献
                    ///note: 对拦截行为有很大影响 建议调参 或者双方距离之差可以用非线性 当前估值的话范围0.2——1.2 (geatpy)
                    val -= _cost_attack_weight * max(self_dis[i] - enemy_dis[1 - i], 4) *
                               (double) cost_attack(tmpside, i, tank[1 - tmpside][1 - i].x,
                                                    tank[1 - tmpside][1 - i].y);///防止距离太近时往旁边躲
...
//        inline bool longtime() { return long_time; }
        ...
        Bot(bool ltime = false,double parameters[] = nullptr){
            long_time =ltime;
#ifdef TRAINING_MODE
            _opposite_can_fire_advantage = parameters[0];
            _self_distance_weight = parameters[1];
            _depth_weight = parameters[2];
            _self_near_base_weight = parameters[3];
        ...
        void Update(Battlefield &f, Json::Value &request, int *responses) {
            ...
            for (unsigned j = 0; j < 2; j++) {
                if (f.tank[enemy][j].steps_in_forest >= 1) {
                    if (f.tank[enemy][j].action != -2) {
                        ...
                        for (int m = 0; m < 9; m++) {
                            for (int n = 0; n < 9; n++) {///当前位置是树林，周围四个的树林也要更新为可能存在敌人
            ...
            for (unsigned j = 0; j < request["destroyed_tanks"].size(); j += 2) {
            ...
            int mark = 0;
            for (unsigned j = 0; j < request["destroyed_blocks"].size(); j += 2) {
        ...
        Json::Value bot_main(Json::Value input) {
            ...
            if (!long_time) {
                for (unsigned i = 1; i < input["requests"].size(); i++) {
                    int act[2] = {
