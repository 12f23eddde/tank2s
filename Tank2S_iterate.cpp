...
struct Battlefield {
	...
	double preference[2][81] = {{0}};///在本地图（已经经过locate enemy简化为tank2情形），采取各个行动的可能性
	double weighted_sum[2][81] = {{0}};///经过preference加权以后得到的预期收益
    void print_state(){
        ///用于调试输出地图
#ifdef PRINT_STATE
        for (int y = 0; y < 9; y++)
    ...
    inline double opposite_eval(int tmpside = self){
        ...
        for(int i=0;i<2;i++){
            if(opposite[i][0] & opposite[i][1]){///自己被敌人夹击,几乎必死无疑
                ...
                eval -= 8.75025;///如果敌人有两辆甚至一辆坦克没有炮弹，大抵还有救， 这里减是额外惩罚
	...
	double evaluate(bool tmpside = self){///针对某一方进行估值，默认对己
        ...
        val += 50  - 1.2 * (double)(self_dis[0] + self_dis[1]);
        val += 1.2 * sgn(enemy_min - self_min) * exp(enemy_min - self_min) / min( min(self_min, enemy_min) + 1 , 5 );///不能让敌人太近
        val += 0.15529628 * (double)(depth(tank[tmpside][1]) + depth(tank[tmpside][0]));///2s的纵深可以适当地价值更高
        ///这个价值可以适当地大一些 大概 1 —— 5 的范围 (geatpy)
        val +=  1.58948026 * exp(6.90892392 - self_min);///离得越近向前走的价值就越大
#ifdef EV_DEBUG
        cout << "p2:" << val << endl;
#endif
        ///关于坦克如何移动到此结束
        ...
        val += 0.7 * double((tank[tmpside][0].n_fire_rounds!=0) + (tank[tmpside][1].n_fire_rounds!=0));
        ...
        for(int i=0;i<2;i++){///己方i
            for(int j=0;j<2;j++){///敌方j
                if(overlap(tank[tmpside][i], tank[!tmpside][j])){
                    if(self_dis[i] - enemy_dis[j] >= -1) val -= 20;///这里采取保守策略，尽可能不出现重叠
                    ///tank2s采取冲锋策略 tank2可以保证不输 但2s不行 这里赌一把
                    else val += 5;///己方离得近的话大概可以冲了
                }
            }
        }
        ...
        for(int i=0;i<2;i++){
            ...
            if(self_dis[i] > enemy_dis[1-i]){///只考虑和自己相对的坦克
                ...
                val -= 0.5*max(self_dis[i] - enemy_dis[1-i],4)*(double)cost_attack(tmpside, i, tank[1-tmpside][1-i].x, tank[1-tmpside][1-i].y);///防止距离太近时往旁边躲
                ///敌人距离扣分
                //val -= (10 - enemy_dis[1-i]);
        ...
        for(int i=0;i<2;i++) {
            if (tank[tmpside][i].killed){
                ...
                if(!Tank::exposed[i]) val -= 35;///算是对己方在树林里作出补偿 因为敌方估值是按敌方上帝视角来算的
                else val -= 50;
            }
            if (tank[1-tmpside][i].killed) val += 50;
        }
#ifdef DEBUG
	...
	Action self_search(vector<Battlefield> & possible_maps){
	    ///self search和enemy search相当于iterate的第一步以及准备工作
	    generate_move(self);
	...
	double enemy_search(Action & act){
	    ...
	    for(int i=0;i<enemy_act.size();i++){
            ...
            values[act.pack()][enemy_act[i].pack()] = sigmoid(temp_ev);
	...
	void iterate(){
	    ///当self search 和enemy search运行完成一遍以后才可以调用
	    ///此时self act和enemy act都已经生成
	    if(iter_count == 0){
	        ///迭代初始化
	        double pre = 1.0/(double)self_act.size();
	        for(auto & i : self_act){
	            preference[self][i.pack()] = pre;
	        }
	        pre = 1.0/(double)enemy_act.size();
            for(auto & i : enemy_act){
                preference[enemy][i.pack()] = pre;
            }
            ///首先假定己方和敌方采取各种行动的可能性都是1/n，即等可能
            int best_pos = 0;
            for(int i=0;i<self_act.size();i++){
                for(auto & j : enemy_act){
                    weighted_sum[self][self_act[i].pack()] += values[self_act[i].pack()][j.pack()] * preference[enemy][j.pack()];
                }
                if(weighted_sum[self][self_act[i].pack()] > weighted_sum[self][self_act[best_pos].pack()]){
                    best_pos = i;
                }
            }
            double best_pre = (1.0 - learning_rate) * preference[self][self_act[best_pos].pack()] + learning_rate;
            double rate = (1.0 - best_pre)/(1.0 - preference[self][self_act[best_pos].pack()]);
            ///第一遍更新己方的weighted sum, 顺带更新preference
            for(int i=0;i<self_act.size();i++){
                if(i==best_pos) preference[self][self_act[i].pack()] = best_pre;
                else preference[self][self_act[i].pack()] *= rate;
            }
            iter_count++;
	    }
	    else{
            ///首先用上次迭代或者迭代初始化时得到的己方preference去更新敌方的preference和weighted sum
            int best_pos = 0;
            for(int i=0;i<enemy_act.size();i++){
                for(auto & j : self_act){
                    weighted_sum[enemy][enemy_act[i].pack()] += values[j.pack()][enemy_act[i].pack()] * preference[self][j.pack()];
                }
                if(weighted_sum[enemy][enemy_act[i].pack()] < weighted_sum[enemy][enemy_act[best_pos].pack()]){
                    best_pos = i;
                }
            }
            double best_pre = (1.0 - learning_rate) * preference[enemy][enemy_act[best_pos].pack()] + learning_rate;
            double rate = (1.0 - best_pre)/(1.0 - preference[enemy][enemy_act[best_pos].pack()]);
            for(int i=0;i<enemy_act.size();i++){
                if(i==best_pos) preference[enemy][enemy_act[i].pack()] = best_pre;
                else preference[enemy][enemy_act[i].pack()] *= rate;
            }
            ///再反过来用新的到的敌方的几率更新己方
            best_pos = 0;
            for(int i=0;i<self_act.size();i++){
            ...
            best_pre = (1.0 - learning_rate) * preference[self][self_act[best_pos].pack()] + learning_rate;
            rate = (1.0 - best_pre)/(1.0 - preference[self][self_act[best_pos].pack()]);
            ///第一遍更新己方的weighted sum, 顺带更新preference
            for(int i=0;i<self_act.size();i++){
                if(i==best_pos) preference[self][self_act[i].pack()] = best_pre;
                else preference[self][self_act[i].pack()] *= rate;
...
/*	    srand(pt->rounds);
        ...
        Action best;
        vector<Battlefield> possible_maps;
        pt->locate_enemy(possible_maps);
        best = pt->self_search(possible_maps);
        while((clock() - start)*1000/CLOCKS_PER_SEC < 930){
            for(int i=0;i<possible_maps.size();i++){
                possible_maps[i].iterate();
            }
        }
        for(int i=0;i<possible_maps.size();i++){
            for(int j=0;j<pt->self_act.size();j++){
                pt->self_act[j].preference += possible_maps[i].preference[self][pt->self_act[j].pack()];
            }
        }
        sort(pt->self_act.begin(), pt->self_act.end(), prior);
#ifdef DEBUG
        for(int i=0;i<pt->self_act.size();i++) {
            cout << i << ": " << pt->self_act[i][0] << ' ' << pt->self_act[i][1] << ' ' << pt->self_act[i].preference << endl;
        }
#endif
        best = pt->self_act[0];
